...
export default function Molecules() {
  ...
  async function addSmiles(e) {
    ...
    try {
      ...
      await moleculeApi.add(data)
      setMsg('Molecula adicionada')
      setForm({ name: '', smiles: '', target_protein_id: '' })
      loadData()
  ...
  async function uploadCsv(e) {
    ...
    setLoading(true)
    try {
      const res = await moleculeApi.uploadCsv(fileInput.files[0])
      setMsg(`${res.data.count} moleculas importadas`)
      setTab('list')
      loadData()
    } catch (e) { setMsg('Erro: ' + (e.response?.data?.detail || e.message)) }
    setLoading(false)
  ...
  async function generateMolecules(e) {
    ...
    try {
      const data = {
        seed_smiles: genForm.seed_smiles,
        n_molecules: parseInt(genForm.n_molecules),
        target_protein_id: genForm.target_protein_id ? parseInt(genForm.target_protein_id) : null,
      }
      const res = await moleculeApi.generate(data)
      setGenResults(res.data)
      setMsg(`${res.data.count} moleculas geradas por IA`)
      loadData()
    } catch (e) { setMsg('Erro: ' + (e.response?.data?.detail || e.message)) }
    setLoading(false)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Moleculas</h1>
      ...
      {tab === 'list' && (
        <Card
          title={`Moleculas (${molecules.length})`}
          actions={
            <div className="flex gap-2">
              <button onClick={() => exportData('json')} className="text-xs bg-gray-100 text-gray-600 px-3 py-1 rounded hover:bg-gray-200">Exportar JSON</button>
              <button onClick={() => exportData('csv')} className="text-xs bg-gray-100 text-gray-600 px-3 py-1 rounded hover:bg-gray-200">Exportar CSV</button>
            </div>
          }
        >
          {molecules.length === 0 ? (
            <p className="text-gray-400 text-sm">Nenhuma molecula. Use "Inicializar Banco" no Dashboard ou adicione manualmente.</p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-gray-500 border-b">
                    ...
                    <th className="pb-2">MW</th>
                    <th className="pb-2">LogP</th>
                    <th className="pb-2">Lipinski</th>
                ...
                <tbody>
                  {molecules.map(m => (
                    <tr key={m.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="py-2">{m.molecular_weight?.toFixed(1) || '-'}</td>
                      <td className="py-2">{m.logp?.toFixed(2) || '-'}</td>
                      <td className="py-2">{m.lipinski_pass === true ? '\u2714' : m.lipinski_pass === false ? '\u2718' : '-'}</td>
                      <td className="py-2 text-xs text-gray-400">{m.source}</td>
        ...
        <Card title="Adicionar Molecula (SMILES)">
          <form onSubmit={addSmiles} className="space-y-4 max-w-lg">
            ...
            <div>
              ...
              <input className={inputCls} placeholder="Ex: c1ccc2ncccc2c1" value={form.smiles}
                onChange={e => setForm({ ...form, smiles: e.target.value })} required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Proteina Alvo (opcional)</label>
        ...
        <Card title="Importar CSV">
          <form onSubmit={uploadCsv} className="space-y-4 max-w-lg">
            <div>
              ...
              <input id="csv-file" type="file" accept=".csv" className={inputCls} required />
              <p className="text-xs text-gray-400 mt-1">Colunas esperadas: name (ou nome), smiles (ou SMILES)</p>
            </div>
        ...
        <div className="space-y-6">
          <Card title="Gerar Novas Moleculas com IA">
            <form onSubmit={generateMolecules} className="space-y-4 max-w-lg">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">SMILES Semente</label>
                <input className={inputCls} placeholder="Ex: COc1cc(NC(C)CCCN(CC)CC)c2ncccc2c1"
                  value={genForm.seed_smiles} onChange={e => setGenForm({ ...genForm, seed_smiles: e.target.value })} required />
                <p className="text-xs text-gray-400 mt-1">Molecula base para geracao de analogos</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Quantidade</label>
                <input type="number" min="1" max="50" className={inputCls}
                  value={genForm.n_molecules} onChange={e => setGenForm({ ...genForm, n_molecules: e.target.value })} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Proteina Alvo (opcional)</label>
              ...
              <button type="submit" disabled={loading} className={btnCls}>{loading ? 'Gerando...' : 'Gerar Moleculas'}</button>
            </form>
          </Card>
            ...
            <Card title={`Moleculas Geradas (${genResults.count})`}>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {genResults.molecules.map((m, i) => (
                      <tr key={i} className="border-b border-gray-50">
                        <td className="py-2">{i + 1}</td>
                        <td className="py-2 text-xs text-gray-600 max-w-[300px] truncate" title={m.smiles}>{m.smiles}</td>
                        <td className="py-2 text-xs">{m.strategy}</td>
                        <td className="py-2">{m.validation?.molecular_weight?.toFixed(1) || '-'}</td>
                        <td className="py-2">{m.validation?.lipinski_pass ? '\u2714' : '\u2718'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
