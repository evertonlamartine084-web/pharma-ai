...
export default function Proteins() {
  ...
  const [tab, setTab] = useState('list') // list, upload, sequence, alphafold
  ...
  async function loadProteins() {
    try {
      const res = await proteinApi.list()
  ...
  async function selectProtein(p) {
    ...
    setPdbData(null)
    if (p.pdb_data) {
      setPdbData(p.pdb_data)
    } else {
      try {
        const res = await proteinApi.getPdb(p.id)
  ...
  async function uploadPdb(e) {
    ...
    setLoading(true)
    try {
      await proteinApi.uploadPdb(fileInput.files[0], form.name, form.organism)
      setMsg('Proteina carregada com sucesso')
      setTab('list')
      loadProteins()
  ...
  async function addSequence(e) {
    ...
    setLoading(true)
    try {
      await proteinApi.addSequence({ name: form.name, sequence: form.sequence, organism: form.organism })
      setMsg('Sequencia adicionada')
      setTab('list')
      loadProteins()
    } catch (e) { setMsg('Erro: ' + (e.response?.data?.detail || e.message)) }
    setLoading(false)
  ...
  async function fetchAlphafold(e) {
    ...
    setLoading(true)
    try {
      await proteinApi.fetchAlphafold({ uniprot_id: form.uniprot_id, name: form.name })
      setMsg('Estrutura AlphaFold obtida com sucesso')
      setTab('list')
      loadProteins()
    } catch (e) { setMsg('Erro: ' + (e.response?.data?.detail || e.message)) }
    setLoading(false)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Proteinas</h1>
      ...
      {tab === 'list' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card title={`Proteinas (${proteins.length})`}>
            {proteins.length === 0 ? (
              <p className="text-gray-400 text-sm">Nenhuma proteina cadastrada. Use "Inicializar Banco" no Dashboard.</p>
            ) : (
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {proteins.map(p => (
                  <button key={p.id} onClick={() => selectProtein(p)}
                    className={`w-full text-left p-3 rounded-lg border transition-colors ${selected?.id === p.id ? 'border-primary-400 bg-primary-50' : 'border-gray-200 hover:border-gray-300'}`}>
                    <p className="font-medium text-sm text-gray-800">{p.name}</p>
                    <p className="text-xs text-gray-400">{p.organism} | Fonte: {p.source}</p>
                  </button>
                ))}
              </div>
            )}
          ...
          <Card title={selected ? selected.name : 'Visualizacao 3D'}>
            ...
            {selected && (
              <div className="mt-4 text-sm text-gray-600 space-y-1">
                <p><span className="font-medium">Organismo:</span> {selected.organism}</p>
                <p><span className="font-medium">Fonte:</span> {selected.source}</p>
                {selected.sequence && <p><span className="font-medium">Sequencia:</span> {selected.sequence.substring(0, 80)}...</p>}
              </div>
            )}
          </Card>
        </div>
        ...
        <Card title="Upload de Arquivo PDB">
          <form onSubmit={uploadPdb} className="space-y-4 max-w-lg">
            ...
            <div>
              ...
              <input className={inputCls} value={form.organism} onChange={e => setForm({ ...form, organism: e.target.value })} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Arquivo PDB</label>
              <input id="pdb-file" type="file" accept=".pdb" className={inputCls} required />
            </div>
      ...
      {tab === 'sequence' && (
        <Card title="Adicionar Sequencia FASTA">
          <form onSubmit={addSequence} className="space-y-4 max-w-lg">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Nome da Proteina</label>
              <input className={inputCls} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Organismo</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Sequencia</label>
              <textarea className={inputCls} rows={6} placeholder="MAQYDKLVIGAGAR..." value={form.sequence}
        ...
        <Card title="Buscar Estrutura no AlphaFold">
          <form onSubmit={fetchAlphafold} className="space-y-4 max-w-lg">
            <div>
              ...
              <input className={inputCls} placeholder="Ex: Q01782" value={form.uniprot_id}
                onChange={e => setForm({ ...form, uniprot_id: e.target.value })} required />
            </div>
            <div>
            ...
            <p className="text-xs text-gray-400">Busca a estrutura predita na base de dados do AlphaFold (EBI).</p>
          </form>
        </Card>
      )}
    </div>
